...
public:
  ...
  __host__ __device__ sphere(vect c, double r, color col){
    center = c;
    radius = r;
    color_val = col;
  }
  ...
  // method functions
  virtual __host__ __device__ vect getSphereCenter() {return center;}
  virtual __host__ __device__ double getSphereRadius() {return radius;}
  virtual __host__ __device__ color getColor() {return color_val;}

  virtual __host__ __device__ vect getNormalAt(vect point){
    vect normal = point.vectAdd(center.negative()).normalize();
    return normal;
  }

  virtual __host__ __device__ double findIntersection(ray r){
